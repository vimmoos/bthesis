...
Ideally, the reasons for performing such processes are:(*Note* this
...
** Autoencoder
   ...
   *decode* must be approximately the inverse of the *encode*[fn:ae_saferl]. The second one is that
   $$ m << n $$
   #+begin_info
   *NOTE* when the second constraint is satisfied, the autoencoder is
   ...
   known as *Latent space* which dimensionality is $\mathbb{R}^m$.
...
** Isomap
   ...
   *Note* Geodesic is the shortest path between two points on the
   ...
   1. Construct a neighbourhoods graph (equivalent to the first step of
      LLE)
   2. Compute the shortest path between points (using either Dijkstra's or
...
Another interesting point made in [fn:darla] is that all Deep
...
representation. Particularly the *explicitness* characteristic, since
...
*** Mathematical Background and explaination of Disentangled representations
...
If $f$ has other properties see [fn:def_dis_repr]
So we can say that $\mathcal{Z}$ is disentangled with respect to a
decomposition $\mathcal{G} = \mathcal{G_i} \times ... \times
\mathcal{G_n}$ if:
+ exists $\cdot$ such that $\cdot: \mathcal{G} \times \mathcal{Z}
  \rightarrow \mathcal{Z}$
+ exists $f$ such that $f:\mathcal{W} \rightarrow \mathcal{Z}$ is a
  $\mathcal{G}$ -morphism or equivariant map.
+  exists a decomposition of $\mathcal{Z} = \mathcal{Z_1} \times
  ... \times \mathcal{Z_n}$ and a disentangled group's action which
  act on it $\cdot$ such that $\cdot_i : \mathcal{G_i} \times
  \mathcal{Z_i} \rightarrow \mathcal{Z_i}$
...
[[https://en.wikipedia.org/wiki/Calculus_of_variations][Variational Calculus]]

The main reference of this overview is the paper [fn:varinf_review]
...
$q(z)$. *Note* simple here just means we have an analytical form of
the distribution
...
We can rearrange the equation as following: (*note* expectation in a
...
Directly quoting from  [fn:varinf_review]
#+begin_verse
...
The Variational autoencoder is a variation of the standard [[*Autoencoder][Autoencoder]]
architecture.
...
#+CAPTION: Variational autoencoder vs Standard Autoencoder
#+attr_html:  :width 600em
[[file:resources/vaevsae.png]]
...
The reconstruction term will be the same as before (e.g the L_2
...
#+begin_info
*NOTE* having two terms in the loss function clearly remark the fact
...
have two properties for the latent space distribution: *Continuity*
(two close points in the latent space must be mapped to close points in
the output)
and *Completeness* (sampling from the latent distribution must returns
...
Some reference [fn:original_vae][[https://towardsdatascience.com/understanding-variational-autoencoders-vaes-f70510919f73][Understanding Variational Autoencoders]]
...
prior ($p(z)$).
In this way, this architecture achieve similar results to the *VAE*
architecture. The main advantage over the *VAE* is that we do
not need a functional form of the prior to enforce it. We
only need to be able to sample from it.
The error function of this architecture is quite similar to the *VAE*
...
** \beta-VAE
The \beta-VAE is a variation on the *VAE* architecture that slightly changes the optimization problem by mutating the objective function. In
particular, it adds a hyperparameter \beta to the =KL-divergence= in
order to arbitrarily force the latent space distribution to match the
prior. With \beta = 1 we have the same formula of the *VAE* meanwhile,
with \beta >> 1 the KL constraint has a higher force. In this way,
following the paper [fn:b-vae] we will have more disentangled
representations.
...
** Info VAE
The InfoVAE was first presented in [fn:infovae].
This architecture tries to make more explicit the *ELBO* tradeoff that
in other architecture are implicit. Doing so they add a new term and 2
hyperparameter \alpha and \lambda.
They also introduce a new term in the *ELBO* objective which is the
...
*Note* the 6th one is about unsupervised disentaglement of neural
networks
*** Variational Inference: A Review for Statisticians[fn:varinf_review]
[fn:varinf_review]
[[file:background/VariationalInference.pdf]]
...
** Autoencoders:
*** Recent advances in Autoencoder-based Representation Learning [fn:ae_review]
[fn:ae_review]
[[file:background/AE_review.pdf]]
...
**** Summary
      + The concept of causuality seems pretty interesting in
        ...
        however, left for future re
*** Life-Long Disentangled representation learning with Cross-domain Latent Homologies [fn:vase]
[fn:vase]
[[file:background/VASE.pdf]]
...
*Todo* search for further research using such method
*Todo* read again and understand deeply the math.

*** TODO Unsupervised model selection for variational disentangled representation learning [fn:select_vae]
[fn:select_vae]
[[file:background/select_VAEs.pdf]]
...
*** Disentangling Disentanglement in Variational Autoencoders [fn:dis_dis_vae]
[fn:dis_dis_vae]
[[file:background/dis_disVAE.pdf]]


**** Summary
+ Take another approach to disentangling representations.
+ decomposition as opposed to disentanglement
+ Achieve a pretty similarity objective to the [fn:infovae]
*** TODO Unsupervised State Representation Learning in Atari [fn:repr_atari]
[fn:repr_atari]
[[file:background/representation_atari.pdf]]


**** Summary
** Continual Learning:
*** TODO Generative Models from the perspective of Continual Learning [fn:gen_cl]
[fn:gen_cl]
[[file:background/continual_learning.pdf]]



**** Summary
*** TODO Embracing Change: Continual Learning in Deep Neural Networks [fn:cl_dnn]
[fn:cl_dnn]
[[file:background/clearning_DNN.pdf]]



**** Summary

*** TODO Continual learning for robotics: Definition,framework,learning strategies, opportunities and challenges [fn:cl_robotics]
[fn:cl_robotics]
[[file:background/cl_for_robotics.pdf]]


**** Summary
*** TODO Continual Unsupervised Representation Learning [fn:cl_repr]
[fn:cl_repr]
[[file:background/c_represent_learning.pdf]]
...
*** TODO Explainability in deep reinforcement learning [fn:exp_drl]
[fn:exp_drl]
[[file:background/explainability_DRL.pdf]]

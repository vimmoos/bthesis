...
representatiton is based on group theory and group representation
theory(all the mathematical theory in this paragraph was summaried
...
Disentagled group of action is.
...
$\mathcal{X}$
Final note, if $\mathcal{X}$ has additional structure we would like
the subaction to preserve such structure. So for example is $\cdot$ is
...
The core problem is to approximate a probabability density which can
...
$$p(x) =\int p(z,x) dz$$
#+begin_info
*NOTE* in a lot of cases the =evidence= integral is unavaible in closed
 form and therefore the integral is most of the time intractable (also
 in the case we can rewrite the formula most of the time the time we
...
framework, we first need to find how to asses the "godness of the
fit".
In other words, we need to find a measure that tell us how close
$q(z)$ is to $p(z|x)$.
...
in negative values.
Another important point is that $log(p(x))$ will be a costant since it
does not change given the dataset (i.e. the obeserved variables)

The =KL-divergence= is by definition something positive. Therefore,
$\mathcal{L}(q)$ must be negative in order for the formula to have
sense.
...
So now instead of minimizing the =KL-divergence= we can maximizing the
=ELBO=.
...
part: reconstruction error term and regularization term.
The reconstruction term will be the same as before (e.g the L_2
between the input and the output)
The regularization term, on the other hand, it will be the
...
(two close points in the latent space must be mapped to close point in
the output)
and *Completness* (sampling from the latent distribution must returns
some "meaningfull" output).

As we can see, we are entering into a probablistic framework. Here,

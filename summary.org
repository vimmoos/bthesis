...
However, this architecture exploit a completely different mechanism.
It useses adversarial learning to force such distribution.
...
($p(z)$) and the actual latent space distribution ($q(z|x)$). In an
adversarial learning process, the encoder is encourage to fool the
...
architecture. The main advantage over the *VAE* is that we actually do
not need a functional form of the prior in order to enforce it. We
...
** \beta-VAE
The \beta-VAE is a variation on the *VAE* architecture slightly change
the optimization problem  by mutating the error function. In
particular, it adds a hyperparameter \beta to the =KL-divergence= in
order to arbitraryly force the latent space distribution to match the
...
follwing the paper [fn:b-vae] we will have more disentangled
representations.
In the same paper, they also shows that \beta is not stricly bound
...
The \lambda hyperparameter, on the other side, it should control the KL
divergence.
...
capture all the prievious seens autoencoders architectures.
